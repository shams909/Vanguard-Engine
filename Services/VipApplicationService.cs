...
public class VipApplicationService : IVipApplicationService
{
    ...
    public VipApplicationService(IUnitOfWork unitOfWork, IUserService userService, IRoleService roleService)
    {
    ...
    {
        return await _unitOfWork.VipApplications.GetPendingApplicationAsync(clientId);
    }

    public async Task<List<VipApplication>> GetMyApplicationsAsync(string clientId)
    {
        return await _unitOfWork.VipApplications.GetByClientIdAsync(clientId);
    }

    public async Task<List<VipApplication>> GetAllApplicationsAsync()
    {
        return await _unitOfWork.VipApplications.GetAllAsync();
    }

    public async Task<List<VipApplication>> GetPendingApplicationsAsync()
    {
        return await _unitOfWork.VipApplications.GetByStatusAsync("Pending");
    }
