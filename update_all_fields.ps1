...
$response = Invoke-RestMethod -Uri "$endpoint/databases/$databaseId/collections/$collectionId/documents?queries[]=limit(500)" -Method Get -Headers $headers
...
foreach ($doc in $documents) {
    if ($doc.email -match "^guard\d+@gmail\.com$") {
        ...
        try {
            ...
            Write-Host "Updated $($doc.email) with RoleID, Address ($address), and Phone ($phoneNumber)"
        } catch {
